...
async function fetchViaLibraryAPI(authToken) {
  ...
  const titles = allRecords
    .map(r => r.sandboxName || r.catalogItem?.title || r.title)
    .filter(t => t && t.length > 1 && !t.match(/^[a-f0-9-]{32,}$/i));
  info(`Library API OK — ${titles.length} titles`);
  return titles;
}
